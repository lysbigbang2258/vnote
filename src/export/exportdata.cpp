...
void ExportOption::fromJson(const QJsonObject &p_obj)
{
    ...
    auto htmlOption = p_obj["html_option"].toObject();
    if (!htmlOption.isEmpty()) {
        m_htmlOption.reset(new ExportHtmlOption());
        m_htmlOption->fromJson(htmlOption);
    }

    auto pdfOption = p_obj["pdf_option"].toObject();
    if (!pdfOption.isEmpty()) {
        m_pdfOption.reset(new ExportPdfOption());
        m_pdfOption->fromJson(pdfOption);
    }
}
...
bool ExportOption::operator==(const ExportOption &p_other) const
{
    ...
    if (m_htmlOption && p_other.m_htmlOption) {
        ret = *m_htmlOption == *p_other.m_htmlOption;
    } else if (m_htmlOption || p_other.m_htmlOption) {
        return false;
    }
    ...
    if (m_pdfOption && p_other.m_pdfOption) {
        ret = *m_pdfOption == *p_other.m_pdfOption;
    } else if (m_pdfOption || p_other.m_pdfOption) {
        return false;
    }
